...
### Requirement: 升級時發放天賦點
系統 SHALL 在角色升級時（`settleRunRewards` 結算的每一次等級提升）額外發放 3 點 `talentPoints`，與既有 `unspentAttributePoints`（每級 +1）並行發放，互不影響。

#### Scenario: 單次升 1 級
- **WHEN** 一次冒險結算讓角色從 5 級升到 6 級
- **THEN** 該角色 `talentPoints` 增加 3，`unspentAttributePoints` 增加 1

#### Scenario: 單次結算連續升多級
- **WHEN** 一次冒險結算的 EXP 讓角色一口氣從 5 級升到 8 級（升 3 級）
- **THEN** 該角色 `talentPoints` 增加 9（3 級 × 3 點）
