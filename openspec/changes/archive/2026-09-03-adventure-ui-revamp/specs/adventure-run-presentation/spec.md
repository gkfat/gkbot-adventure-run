...
### Requirement: 點擊推進顯示走路動畫與背景移動感
系統 SHALL 在玩家點擊「推進」時，播放角色走路動畫，並使背景呈現移動感；動畫播放期間不得阻擋後續 API 回應到達後的狀態更新。

#### Scenario: 推進觸發走路動畫
- **WHEN** 玩家點擊「推進」且尚未有走路動畫在播放
- **THEN** 角色顯示背面走路動畫幀序列，背景同步呈現移動感，動畫結束後畫面切換至新節點狀態

#### Scenario: 推進 API 回應早於動畫播放完畢
- **WHEN** 玩家點擊「推進」後，`advance()` 的 API 回應在走路動畫尚未播放完畢前就已到達
- **THEN** 系統不因動畫尚未結束而阻塞或延遲新節點狀態的顯示邏輯（動畫效果與狀態更新為獨立關注點）
