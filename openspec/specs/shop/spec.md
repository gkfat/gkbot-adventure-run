...
#### Scenario: 技能碎片商品的 skillId 屬於角色自身職業
- **WHEN** 查詢某角色的當日商店，其中包含 `type = 'SKILL_FRAGMENT'` 的商品
- **THEN** 該商品的 `skillId` 必屬於該角色 `archetypeId` 對應的 `CHARACTER_SKILLS` 清單，不會出現其他職業的技能
...
### Requirement: 購買商店商品
系統 SHALL 於購買時檢查商品未售出且玩家對應貨幣（依商品的 `currency` 決定 gold 或 gems）資源足夠，扣款並發放物品實體；商品一旦售出即標記 `sold`，不可重複購買。購買請求 SHALL 只需指定 `slotId`，不需另外指定貨幣類型——貨幣別由該 slot 本身的 `currency` 決定。當商品 `type = 'SKILL_FRAGMENT'` 時，購買結算 SHALL NOT 走「加入永久背包/裝備槽位」路徑，改為直接為該角色 `skillFragments[skillId]` 增加該商品的 `fragmentAmount`，`destination` 參數對此商品類型 SHALL 被忽略。
...
#### Scenario: 購買技能碎片商品不佔用背包容量
- **WHEN** 玩家購買一個 `type = 'SKILL_FRAGMENT'` 的商品，對應貨幣資源足夠，即使角色永久背包已達 500 格上限
- **THEN** 購買仍成功，該商品標記 sold，角色對應貨幣扣除 `price`，`skillFragments[skillId]` 增加 `fragmentAmount`，不檢查、不受背包容量限制

#### Scenario: 技能碎片商品已售出
- **WHEN** 玩家對已標記 sold 的 `SKILL_FRAGMENT` 商品呼叫購買
- **THEN** 系統回傳 409，不扣款、不增加碎片

#### Scenario: 購買回應附上可直接顯示的技能名稱與圖示
- **WHEN** 玩家成功購買一個 `type = 'SKILL_FRAGMENT'` 的商品
- **THEN** 回應的 `skillFragment` 除 `skillId`/`amount` 外，另包含該技能的 `name`/`icon`（查自 `CHARACTER_SKILLS`），前端 SHALL 依此立即彈出結果 dialog 顯示「取得技能碎片：{name} +{amount}」，不需要額外呼叫技能查詢 API 才能顯示名稱
