...
- **`d-flex` 用於「格狀/多欄排版」時，優先改用 `v-row`/`v-col`**：判斷準則——多個「欄位」並排、需要響應式欄寬（`cols`/`sm`/`md`）、或本質是網格排版（例如固定數量的統計數據並排欄位、多欄表單、卡片網格）時用 `v-row`/`v-col`；單純置中對齊、頭尾對齊（`justify-space-between`）、垂直堆疊（`flex-column`）、inline 小元件（chip/icon 容器）、或項目數量會變動而需自適應寬度置中的清單，仍維持 `d-flex`。`v-row`/`v-col` 有預設負 margin（gutter）與斷行行為，緊湊的像素風格 UI（如 `.pixel-slot` 格子、hp-bar、按鈕列）硬套會破壞版面，這類情境不套用。
