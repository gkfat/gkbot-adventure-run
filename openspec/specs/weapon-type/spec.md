...
武器類型（`weaponType`）：`HAND_SLOTS`（`LEFT_HAND`/`RIGHT_HAND`）道具可定義的固定分類屬性（`FIST`/`BLADE`/`BLUNT`/`POLEARM`/`RANGED`），決定該武器歸屬的熟練度累積類型（`weapon-proficiency` capability）與攻擊型態風格；與 `weaponWeightClass`（`weapon-weight-class` capability）為正交軸線，互不取代、互不衍生。
