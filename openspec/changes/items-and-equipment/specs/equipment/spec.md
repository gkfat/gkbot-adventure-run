...
系統 SHALL 支援 8 個固定裝備槽位（HEAD/BODY/SHOES/GLOVES/LEFT_HAND/RIGHT_HAND/NECKLACE/RING）；裝備一件物品時，若目標槽位已有裝備，系統 SHALL 回傳需要替換確認的資訊而非直接覆蓋。

#### Scenario: 裝備到空槽位
- **WHEN** 玩家對背包內某件 `equipSlot = HEAD` 的物品呼叫 `POST /api/character/equip`，且 HEAD 槽位目前為空
...
### Requirement: 卸下裝備
系統 SHALL 提供 `POST /api/character/unequip`，卸下指定槽位的裝備；裝備效果卸下後立即失效（不再計入下次 stats 計算）。
