...
角色依 `weaponType`（`weapon-type` capability）累積的武器熟練度，以及獨立的雙持熟練度：戰鬥命中自動累積 exp、依共用等級曲線升級、套用於 stats 的數值加成、解鎖類型與雙持專屬的戰鬥被動技能，並於角色頁面呈現。
...
- **THEN** 該場戰鬥中雙持被動不再生效，該武器類型自己的被動不受影響
