...
import { claimDailyQuestResponseSchema } from '../../../../../../../shared/schemas/api/quest.schema';
...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        } = await questService.claimDaily(authUser.uid, characterId, questId);
        ...
        return claimDailyQuestResponseSchema.parse(response);
    } catch (error: unknown) {
        logRequest({
            severity: 'ERROR',
            message: 'Failed to claim daily quest',
            method: event.method,
            path: event.path,
